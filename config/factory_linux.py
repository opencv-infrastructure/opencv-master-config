...
class AbiFindBaseCommand(SetPropertyFromCommand):
    ...
    def __init__(self, builder, **kwargs):
        def extractor(rc, stdout, stderr):
            ...
            if isBranch34(builder):
                print 'ABI: fallback to 3.4.16'
                return {'abi_base_file':'/opt/build-worker/abi/dump-3.4.16.abi.tar.gz'}
            else:
                print 'ABI: fallback to 4.5.4'
                return {'abi_base_file':'/opt/build-worker/abi/dump-4.5.4.abi.tar.gz'}
    ...
    def getCandidates(self):
        verString = self.getProperty('commit-description', '3.4.16' if isBranch34(self.build) else '4.5.4')
